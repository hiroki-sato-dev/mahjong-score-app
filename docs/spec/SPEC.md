...
| -------------------- | ------------------------------------------------------ |
| ログイン             | メール＋パスワード認証                                 |
| 部員追加             | 幹部が管理画面から部員を追加（セルフ登録は提供しない） |
...
- 2位・3位・4位: `(rawScore - 30000) / 1000 + ウマ`
- 1位: `-(2位 + 3位 + 4位)` ← 残差（オカ＋端数）を吸収して合計0に
- 返し = 30000点
- ウマ: 1位 +20 / 2位 +10 / 3位 -10 / 4位 -20

**最終P**

- `最終P = 素点P × 50 + チップ枚数 × 100`
- チップなしの半荘では `チップ枚数 × 100` を加算しない

**計算例（チップなし、素点 50000 / 30000 / 15000 / 5000）**

| 順位 | 素点   | 素点P             | 最終P  |
| ---- | ------ | ----------------- | ------ |
| 1位  | 50,000 | +60 ← 残差吸収    | +3,000 |
| 2位  | 30,000 | 0 + 10 = +10      | +500   |
| 3位  | 15,000 | -15 + (-10) = -25 | -1,250 |
| 4位  | 5,000  | -25 + (-20) = -45 | -2,250 |
...
- プッシュ通知
- 外部サービス連携（LINE通知等）
- セルフ新規登録（部活内の招待制とするため、幹部が `/admin/members` から追加する運用）
...
- パスワード入力
- ログインボタン
- ※ セルフ新規登録は提供しない（部員追加は幹部のみ、`/admin/members` から行う）
